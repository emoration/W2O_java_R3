...
public class App {
    public static void main(String[] args) {
        ...
        label:
        while (true) {
            System.out.println("input: ['0': quit, 'q': query weather, 'a': add city, 'u' to update, 'd' to delete]");
            String input = scanner.next();
            switch (input) {
                case "0":
                    break label;
                case "q": {
                    System.out.println("input the cityName to query");
                    ...
                    switch (days) {
                        case "0" -> jsonObject = demo.queryCityToday(cityName);
                        case "1" -> jsonObject = demo.queryCityTomorrow(cityName);
                        case "2" -> jsonArray = demo.queryCityDays(cityName);
                    }
                    if (jsonObject != null) {
                        ...
                        System.out.println("fail, haven't found");
                    }
                    break;
                }
                case "a": {
                ...
                case "d": {
                    System.out.println("input the cityName to delete");
                    String cityName = scanner.next().toLowerCase();
                    if (demo.deleteCity(cityName)) {
                        System.out.println("success");
                    } else {
                        System.out.println("fail, haven't found");
                    }
